...
def speck_1r_key(btor, nb_bits, data_mask, key_mask, weight, i, key_words):
    """Route the round-key mask to K[i mod m] in the simple linear schedule."""
    u, v = data_mask
    p_arr, q = key_mask
    p_idx = i % key_words
    if p_idx == 0:  # ARK with q branch
        for mi in range(key_words - 1):
            btor.Assert(p_arr[mi][i] == p_arr[mi][i + 1])
            
        btor.Assert(q[i] == (q[i + 1] ^ u[i + 1] ^ v[i + 1]))
    else:  # ARK with p_arr[i % key_words - 1] branch
        p_idx -= 1  # Convert key-word indices 1..m-1 to p_arr indices 0..m-2.
        p = p_arr[p_idx]
        for mi in range(key_words - 1):
            if mi == p_idx: continue
            btor.Assert(p_arr[mi][i] == p_arr[mi][i + 1])
        btor.Assert(q[i] == q[i + 1])

        btor.Assert(p[i] == (p[i + 1] ^ u[i + 1] ^ v[i + 1]))

    return weight

# with considering key-schedule
def speck_quasidifferential_trails(diffs, nb_bits, key_words):
    """Build the SMT model for SPECK with the simple cyclic linear schedule."""
    btor = Boolector()
